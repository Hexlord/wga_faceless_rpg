...
public class Smart_controller : MonoBehaviour
{
    ...
    public Vector2 Aim_speed = new Vector2(2.0f, 2.0f);
    ...
    */
     ...
     */
    public float Look_afk_threshold = 10.0f;

    /*
     * Minimum angle difference in degree to trigger force character rotation
     */
    public float Look_away_threshold = 180.0f;
    ...
    void FixedUpdate()
    {
        ...
        float camera_angle_horizontal = (look_aim.x / 180.0f + 0.5f) * Mathf.PI;
        ...
        player_delta.Set(
            ...
            Mathf.DeltaAngle(player_current.y, look_aim.y));

        if (!is_player_rotating)
        {
            if (Mathf.Abs(player_delta.x) > Look_away_threshold)
            {
            ...
            else
            {
                ...
                } else
                {
                    ...
                    if(look_afk_timer <= 0.0f &&
                        Mathf.Abs(Mathf.DeltaAngle(camera_lo)) > Look_afk_threshold)
                    {
        ...
        if (is_player_rotating)
        {
                ...
                Mathf.Abs(player_delta.y) > Mathf.Epsilon)
            {
                ...
                Quaternion q = Quaternion.Euler(0.0f, player_current.x, 0.0f);
        ...
        float angle = (player_current.y / 180.0f) * Mathf.PI;
