...
public class Smart_controller : MonoBehaviour
{
    ...
    public Vector2 Mouse_sensitivity = new Vector2(3.7f, 3.7f);
    ...
    [Tooltip("Planar XZ distance between camera and player")]
    public float Camera_distance = 3.3f;

    [Tooltip("Vertical distance between camera and player")]
    public float Camera_height = 1.8f;

    [Tooltip("Y distance between camera target and player")]
    public float Camera_target_height = 0.7f;
    ...
    void FixedUpdate()
    {
        ...
        float camera_angle_cos = Mathf.Cos(camera_angle_horizontal);
        float camera_angle_sin = Mathf.Sin(camera_angle_horizontal);

        camera_position.Set(
            player_transform.position.x + camera_angle_cos * Camera_distance,
            player_transform.position.y + Camera_height,
            player_transform.position.z - camera_angle_sin * Camera_distance);
