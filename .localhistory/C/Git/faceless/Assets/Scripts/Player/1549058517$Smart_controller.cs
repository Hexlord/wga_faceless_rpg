...
public class Smart_controller : MonoBehaviour
{
    ...
    */
    ...
    void FixedUpdate()
    {
        ...
        float player_angle_sin = Mathf.Sin(player_current.x / 180.0f + 0.5f) * Mathf.PI;
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (look_afk_timer <= 0.0f &&
                        Mathf.Abs(Mathf.DeltaAngle(player_current.x, camera_look_current.x)) > Look_afk_threshold)
                    {
                        is_player_rotating = true;
                    }
                }
